...
async function setupFilebase() {
  ...
  try {
    ...
    if (missingVars.length > 0) {
      ...
      console.log('🔗 Get your credentials from: https://filebase.com/account/keys');
      process.exit(1);
    }
    ...
    try {
      ...
      const result = await client.uploadMetadata('test/festos-setup.json', testData);
...
`;
    ...
    console.log('\n📋 Summary:');
    console.log(`   Access Key ID: ${process.env.FILEBASE_ACCESS_KEY_ID?.substring(0, 8)}...`);
    console.log(`   Bucket: ${process.env.FILEBASE_BUCKET}`);
    console.log(`   Endpoint: ${process.env.FILEBASE_ENDPOINT || 'https://s3.filebase.com'}`);
