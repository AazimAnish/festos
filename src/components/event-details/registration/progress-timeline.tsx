...
export const ProgressTimeline = memo(function ProgressTimeline({ 
  ...
  const actualProgress = useMemo(() => {
    ...
    if (currentStep >= 1) {
      ...
      const filledRequiredFields = requiredFields.filter(field => formData[field.label]);
  ...
  const progressDescription = useMemo(() => {
    if (currentStep === 1) {
      const remainingFields = form.filter(field => field.required).length - 
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          <span className="font-secondary text-sm font-medium text-foreground">Step {currentStep} of {STEPS.length}</span>
          <span className="text-xs text-gray">{STEPS[currentStep - 1].title}</span>
        </div>
        <span className="text-xs text-gray font-mono">{Math.round(actualProgress)}%</span>
      </div>
      ...
      <div className="flex items-center justify-between">
        {STEPS.map((step) => {
          ...
          const isPartiallyCompleted = step.id === 1 && currentStep >= 1 && formData[form[0]?.label];
          ...
          if (step.id === 1) {
            const requiredFields = form.filter(field => field.required);
            const filledRequiredFields = requiredFields.filter(field => formData[field.label]);
            stepCompleted = filledRequiredFields.length === requiredFields.length;
          } else if (step.id === 2) {
          ...
          return (
            <div key={step.id} className="flex flex-col items-center space-y-2">
              <div className={`relative flex items-center justify-center w-10 h-10 rounded-3xl border-2 transition-all duration-200 ${
                stepCompleted 
                  ? 'bg-primary border-primary text-primary-foreground shadow-lg' 
                  : isActive 
                  ? 'bg-primary/10 border-primary text-primary shadow-md' 
                  : isPartiallyCompleted || isWalletStepCompleted || isTermsStepCompleted
                ...
                {!stepCompleted && (isPartiallyCompleted || isWalletStepCompleted || isTermsStepCompleted) && (
                  <div className="absolute inset-0 rounded-3xl bg-primary/20 animate-pulse"></div>
                )}
              </div>
              <span className={`text-xs font-medium ${
                stepCompleted ? 'text-primary' : isActive ? 'text-primary' : 'text-gray'
              }`}>
                {step.title}
              </span>
