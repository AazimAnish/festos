...
export function CheckInPage({ eventId }: CheckInPageProps) {
  ...
  // Mock attendees data
  const attendees = [
    { id: 1, name: "Alex Chen", email: "alex@example.com", checkedIn: true, ticketType: "General Admission", ticketId: "TICKET-001" },
    { id: 2, name: "Sarah Kim", email: "sarah@example.com", checkedIn: false, ticketType: "VIP", ticketId: "TICKET-002" },
    { id: 3, name: "Michael Rodriguez", email: "michael@example.com", checkedIn: true, ticketType: "General Admission", ticketId: "TICKET-003" },
    { id: 4, name: "Emily Johnson", email: "emily@example.com", checkedIn: false, ticketType: "General Admission", ticketId: "TICKET-004" },
    { id: 5, name: "David Lee", email: "david@example.com", checkedIn: false, ticketType: "VIP", ticketId: "TICKET-005" },
  ];
  
  const [scanError, setScanError] = useState<string | null>(null);
  const [scanSuccess, setScanSuccess] = useState(false);
  const [foundAttendee, setFoundAttendee] = useState<typeof attendees[0] | null>(null);
      ...
      if (ticketData && ticketData.ticketId) {
        ...
        if (matchedAttendee) {
          ...
          toast.success(`Found: ${matchedAttendee.name}`, {
            description: matchedAttendee.checkedIn ? "Already checked in" : "Successfully checked in"
          });
          ...
          setScanError("Invalid ticket ID. This ticket is not registered for this event.");
        ...
        toast.error("Invalid QR code", {
          description: "The scanned code doesn't contain valid ticket information."
        });
      }
    } catch {
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <Link href={`/event/manage/${eventId}`} className="hover:opacity-80 transition-opacity">
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" className="gap-1.5" onClick={() => setShowScanner(!showScanner)}>
        ...
        <FadeIn variant="up" timing="normal">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
            <Card className="p-6 space-y-2">
              <h3 className="font-secondary text-sm text-muted-foreground">Total Attendees</h3>
              <p className="font-primary text-2xl font-bold text-foreground">{eventData.totalAttendees}</p>
            </Card>
            <Card className="p-6 space-y-2">
              <h3 className="font-secondary text-sm text-muted-foreground">Checked In</h3>
              <p className="font-primary text-2xl font-bold text-foreground">{eventData.checkedInAttendees}</p>
            </Card>
            <Card className="p-6 space-y-2">
              <h3 className="font-secondary text-sm text-muted-foreground">Remaining</h3>
                ...
                <div className="max-w-md mx-auto">
                  {scanError && !scanSuccess && (
                    <div className="mb-4 p-3 bg-destructive/10 border border-destructive/20 rounded-lg text-center">
                      <p className="font-secondary text-sm text-destructive">{scanError}</p>
                    </div>
                  )}
                  ...
                  {scanSuccess && foundAttendee && (
                    <div className="mb-4 p-4 bg-success/10 border border-success/20 rounded-lg">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h4 className="font-primary text-base font-semibold text-foreground">{foundAttendee.name}</h4>
                          <p className="font-secondary text-xs text-muted-foreground">{foundAttendee.email}</p>
                        </div>
                        <div className="ml-auto">
                          <span className={`px-2 py-1 rounded-md text-xs ${foundAttendee.checkedIn ? "bg-success/20 text-success" : "bg-muted text-muted-foreground"}`}>
                  ...
                  <div className="relative">
                    ...
                    <div className="absolute top-0 left-0 right-0 z-10 bg-background/80 backdrop-blur-sm p-2 rounded-t-lg">
                      <p className="font-secondary text-xs text-center text-muted-foreground">
                        Camera access required. Please allow permissions when prompted.
                      </p>
                    </div>
                    ...
                    <div className="absolute bottom-0 left-0 right-0 z-10 bg-background/80 backdrop-blur-sm p-2 rounded-b-lg">
                      <div className="flex items-center justify-between">
                        ...
                        <p className="text-xs text-muted-foreground">Scanning for QR codes...</p>
                        <Button 
                          variant="ghost" 
                          size="sm"
                          onClick={() => {
            ...
            <div className="mb-8">
              <div className="border-2 border-yellow-500/30 rounded-lg p-4 bg-yellow-500/10 mb-4">
                <p className="font-secondary text-sm text-center text-yellow-600">
                  Developer tools: Use this QR generator to test the check-in scanning functionality
                </p>
              </div>
              <QrCodeGenerator 
          ...
          <div className="space-y-4">
            <h2 className="font-primary text-xl font-bold text-foreground">Attendees</h2>
            
            <div className="border border-border rounded-lg overflow-hidden">
              <table className="w-full">
                <thead className="bg-muted/30">
                  <tr>
                    <th className="px-4 py-3 text-left font-primary text-sm font-bold text-foreground">Name</th>
                    <th className="px-4 py-3 text-left font-primary text-sm font-bold text-foreground">Email</th>
                    <th className="px-4 py-3 text-left font-primary text-sm font-bold text-foreground">Ticket Type</th>
                    <th className="px-4 py-3 text-right font-primary text-sm font-bold text-foreground">Status</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredAttendees.map((attendee) => (
                    <tr key={attendee.id} className="border-t border-border">
                      <td className="px-4 py-3 font-secondary text-sm">{attendee.name}</td>
                      <td className="px-4 py-3 font-secondary text-sm text-muted-foreground">{attendee.email}</td>
                      <td className="px-4 py-3 font-secondary text-sm">{attendee.ticketType}</td>
                      <td className="px-4 py-3 text-right">
                        <Button
                          variant={attendee.checkedIn ? "default" : "outline"}
                          size="sm"
                          className={`gap-1.5 ${
                            attendee.checkedIn ? "bg-green-600 hover:bg-green-700" : ""
                          }`}
