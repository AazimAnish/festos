...
export function POAPCalendarView({
  poaps,
  onOpenDetails,
  onShare,
}: POAPCalendarViewProps) {
  ...
  const calendarDays = useMemo(() => {
    ...
    for (let i = 0; i < firstDayOfMonth; i++) {
      days.push(
        <div key={`empty-${i}`} className='p-2 bg-muted/10 rounded-md'></div>
      );
    }
      ...
      days.push(
        ...
        >
          <div className='flex items-center justify-between mb-1'>
            <span
              className={`text-sm font-medium ${isCurrentMonth ? 'text-primary' : ''}`}
            >
              {day}
            </span>
            {dayPoaps.length > 0 && (
  ...
  const currentMonthPoaps = useMemo(() => {
    return poaps
      .filter(poap => {
        ...
        return (
          date.getFullYear() === activeYear && date.getMonth() === activeMonth
        );
      })
