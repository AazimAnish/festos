...
export function DateTimePicker({ startDate, endDate, onStartDateChange, onEndDateChange }: DateTimePickerProps) {
      ...
      <div className="flex rounded-xl border border-border overflow-hidden">
        <Popover open={startDateOpen} onOpenChange={setStartDateOpen}>
          <PopoverTrigger asChild>
            ...
            >
              ...
              <span className="text-sm">{startDate ? format(startDate, "MMM d, yyyy") : "Start date"}</span>
            </Button>
          </PopoverTrigger>
        ...
        <Popover open={startTimeOpen} onOpenChange={setStartTimeOpen}>
          <PopoverTrigger asChild>
            <Button
              variant="ghost"
              className={cn("flex-1 h-12 rounded-none justify-start gap-3", !startDate && "text-muted-foreground")}
            >
              <Clock className="h-4 w-4" />
              <span className="text-sm">{startDate ? format(startDate, "h:mm a") : "Start time"}</span>
            </Button>
          </PopoverTrigger>
      ...
      <div className="flex rounded-xl border border-border overflow-hidden">
        <Popover open={endDateOpen} onOpenChange={setEndDateOpen}>
          <PopoverTrigger asChild>
            ...
            >
              <CalendarIcon className="h-4 w-4" />
              <span className="text-sm">{endDate ? format(endDate, "MMM d, yyyy") : "End date"}</span>
            </Button>
          </PopoverTrigger>
          <PopoverContent className="w-auto p-0" align="start">
            <Calendar
              ...
              disabled={(date) => {
                ...
                const startDateOnly = startDate
                  ...
                  : today
                return date.getTime() < Math.max(today.getTime(), startDateOnly.getTime())
              }}
              initialFocus
            />
          </PopoverContent>
        ...
        <Popover open={endTimeOpen} onOpenChange={setEndTimeOpen}>
          <PopoverTrigger asChild>
            <Button
              variant="ghost"
              className={cn("flex-1 h-12 rounded-none justify-start gap-3", !endDate && "text-muted-foreground")}
            >
              <Clock className="h-4 w-4" />
              <span className="text-sm">{endDate ? format(endDate, "h:mm a") : "End time"}</span>
            </Button>
          </PopoverTrigger>
          <PopoverContent className="w-32 p-0" align="start">
            <div className="max-h-60 overflow-y-auto p-1">
