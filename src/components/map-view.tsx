...
export function MapView({ events, isOpen, onClose, onClearFilters }: MapViewProps) {
            ...
            <div class="absolute -bottom-3 left-1/2 transform -translate-x-1/2 bg-background border-2 border-border rounded-lg px-3 py-2 shadow-xl opacity-0 group-hover:opacity-100 transition-all duration-200 pointer-events-none whitespace-nowrap z-10 min-w-[200px]">
              <div class="space-y-2">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-xs font-secondary text-gray">${event.joinedCount} joined</span>
                </div>
                <div class="flex items-center gap-2">
                  <Calendar className="w-3 h-3 text-primary" />
                  <span class="text-xs font-secondary text-gray">${new Date(event.date).toLocaleDateString()}</span>
                </div>
              </div>
            </div>
          </div>
                ...
                <div class="p-4 max-w-sm space-y-4">
                  <div class="space-y-3">
                    <div class="flex items-start justify-between">
                      ...
                      <div class="flex items-center gap-1 text-xs text-gray bg-accent/10 px-2 py-1 rounded-full">
                  ...
                  <div class="flex items-center justify-between pt-2 border-t border-border">
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <span class="inline-flex items-center gap-1 text-xs font-medium bg-accent/10 text-accent px-2 py-1 rounded-full">${event.category}</span>
                    </div>
                  </div>
                </div>
              `)
  ...
  return (
    <>
      ...
      >
        {/* Header */}
        <div className="absolute top-0 left-0 right-0 z-10 bg-background/95 backdrop-blur-md border-b border-border p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Navigation className="w-5 h-5 text-primary" />
              <div>
                <h2 className="font-primary text-lg font-bold">Map View</h2>
                <p className="font-secondary text-sm text-gray">{events.length} events nearby</p>
              </div>
            </div>
            <Button
              variant="ghost"
