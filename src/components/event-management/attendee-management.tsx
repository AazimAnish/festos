...
export function AttendeeManagement({ eventId }: AttendeeManagementProps) {
  ...
  const handleAttendeeAdded = useCallback((newAttendee: {
    name: string;
    email: string;
    ticketType: "General" | "VIP" | "Premium";
    walletAddress?: string;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="font-primary text-xl font-bold text-foreground">Guest Management</h2>
          <p className="font-secondary text-sm text-muted-foreground">
            Manage your event attendees, send communications, and track check-ins
          </p>
        </div>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={() => {
                toast.success('CSV export started. You\'ll receive it via email.');
              }}>
                Export as CSV
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => {
                toast.success('Excel export started. You\'ll receive it via email.');
              }}>
                Export as Excel
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => {
                toast.success('PDF export started. You\'ll receive it via email.');
              }}>
                Export as PDF
              </DropdownMenuItem>
            </DropdownMenuContent>
                  ...
                  <div className="p-2 border rounded-md bg-muted/20">
                    <div className="flex items-center gap-2 flex-wrap">
                      {selectedAttendees.length > 0 ? (
                        ...
                        <span className="text-sm text-muted-foreground">No guests selected</span>
                      )}
                    </div>
                  </div>
                </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <Card className="bg-background border-border/50 rounded-xl hover:shadow-md transition-shadow">
          <CardContent className="p-6">
            <div className="font-primary text-2xl font-bold text-foreground">{stats.total}</div>
            <div className="font-secondary text-sm text-muted-foreground">Total Registrations</div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-background border-border/50 rounded-xl hover:shadow-md transition-shadow">
          <CardContent className="p-6">
            ...
            <div className="font-secondary text-sm text-muted-foreground">Checked In</div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-background border-border/50 rounded-xl hover:shadow-md transition-shadow">
          <CardContent className="p-6">
            ...
            <div className="font-secondary text-sm text-muted-foreground">Not Checked In</div>
          </CardContent>
        </Card>
          ...
          <table className="w-full">
            <thead className="bg-muted/30">
              <tr>
                ...
                <th className="px-4 py-3 text-left font-medium text-sm">Attendee</th>
                <th className="px-4 py-3 text-left font-medium text-sm">Ticket Type</th>
                <th className="px-4 py-3 text-left font-medium text-sm">Purchase Date</th>
                <th className="px-4 py-3 text-left font-medium text-sm">Status</th>
                <th className="px-4 py-3 text-left font-medium text-sm">Check-in Time</th>
                  ...
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {attendee.phone && (
                          <div className="text-xs text-muted-foreground truncate">{attendee.phone}</div>
                        )}
                      </div>
                    </div>
                  </td>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-48">
                          <DropdownMenuItem onClick={() => copyToClipboard(attendee.email)}>
                            <Copy className="w-4 h-4 mr-2" />
                            Copy Email
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => copyToClipboard(attendee.walletAddress)}>
                            <Copy className="w-4 h-4 mr-2" />
                            Copy Wallet
                          </DropdownMenuItem>
                          <DropdownMenuSeparator />
