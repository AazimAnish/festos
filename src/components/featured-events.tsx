...
export default function FeaturedEvents() {
  ...
  const filteredEvents = SAMPLE_EVENTS.filter(event => {
    switch (activeFilter) {
      ...
      case "near":
        // For demo purposes, show events in specific cities
        return ["Mumbai, India", "Bangalore, India"].includes(event.location);
      case "trending":
        // For demo purposes, show events with more than 300 attendees
        return event.joinedCount > 300;
      default:
        return true;
    }
  });
            ...
            {eventFilters.map((filter) => {
              ...
              return (
                <Button
                  ...
                  `}
                >
                  <Icon className={`w-3 h-3 sm:w-4 sm:h-4 mr-1.5 sm:mr-2 ${isActive ? "" : ""}`} />
                  {filter.label}
                </Button>
              );
            })}
          ...
          {displayEvents.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
              ...
              <p className="text-muted-foreground max-w-md">
                We couldn&apos;t find any events matching your current filter. Try another category or check back later.
              </p>
              <Button 
