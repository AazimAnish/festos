...
export function TicketListingDialog({ open, onOpenChange }: TicketListingDialogProps) {
  ...
  }, [selectedTicketId, listingPrice, listingDuration, autoAcceptEnabled, autoAcceptPrice, listTicket, onOpenChange]);

  const selectedTicket = transferableTickets.find(t => t.id === selectedTicketId);
  const platformFee = listingPrice ? (parseFloat(listingPrice) * 0.025).toFixed(3) : "0";
  const netAmount = listingPrice ? (parseFloat(listingPrice) - parseFloat(platformFee)).toFixed(3) : "0";
      ...
      <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          {[
            ...
            const isCompleted = ["configure", "review"].includes(step) && index === 0;
            
            return (
              <div key={stepInfo.key} className="flex items-center">
                <div className={`flex items-center gap-2 ${isActive ? "text-primary" : isCompleted ? "text-success" : "text-muted-foreground"}`}>
                  ...
                  <span className="hidden sm:inline text-sm font-medium">{stepInfo.label}</span>
                </div>
                {index < 2 && (
                  <div className={`w-8 h-0.5 mx-2 ${isCompleted ? "bg-success" : "bg-muted"}`} />
                )}
              </div>
            );
          })}
        ...
        {step === "select" && (
          <div className="space-y-6">
            <div>
              <h3 className="font-primary text-lg font-semibold mb-3">Select a Ticket to List</h3>
              <p className="text-muted-foreground mb-4">
                Choose a transferable ticket from your collection to list for sale.
              </p>
            </div>
            ...
            {transferableTickets.length === 0 ? (
              <div className="text-center py-8 space-y-4">
                ...
                <div>
                  <h3 className="font-primary text-lg font-semibold mb-2">No Transferable Tickets</h3>
                  <p className="text-muted-foreground">
        ...
        {step === "configure" && selectedTicket && (
          <div className="space-y-6">
            <div>
              <h3 className="font-primary text-lg font-semibold mb-3">Configure Your Listing</h3>
              <p className="text-muted-foreground">
                Set your listing price and duration to maximize your chances of selling.
              </p>
            </div>
              ...
              <CardContent className="p-4">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <h4 className="font-primary font-semibold">{selectedTicket.eventName}</h4>
                    <div className="flex items-center gap-2 mt-1 text-sm text-muted-foreground">
                      <Calendar className="w-4 h-4" />
                      {new Date(selectedTicket.eventDate).toLocaleDateString()}
                      <Badge variant="outline" className="ml-2">{selectedTicket.ticketType}</Badge>
                    </div>
                  </div>
                </div>
              </CardContent>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                <Info className="w-4 h-4" />
                                            <span>
                              {parseFloat(listingPrice || "0") > parseFloat(selectedTicket.originalPrice) 
            ...
            <div className="space-y-3">
              ...
              {autoAcceptEnabled && (
                <div className="space-y-2 pl-6">
                  <Label htmlFor="auto-accept-price">Auto-accept price (ETH)</Label>
                  <Input
        ...
        {step === "review" && selectedTicket && (
          <div className="space-y-6">
            <div>
              <h3 className="font-primary text-lg font-semibold mb-3">Review Your Listing</h3>
              <p className="text-muted-foreground">
            ...
            <Card className="bg-muted/20 border-primary/20">
              <CardContent className="p-6 space-y-4">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <h4 className="font-primary text-lg font-semibold">{selectedTicket.eventName}</h4>
                    <div className="flex items-center gap-4 mt-1 text-sm text-muted-foreground">
                      <div className="flex items-center gap-1">
                        <Calendar className="w-4 h-4" />
                        {new Date(selectedTicket.eventDate).toLocaleDateString()}
                    ...
                    <Badge variant="outline" className="mt-2">{selectedTicket.ticketType}</Badge>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 pt-4 border-t border-border/50">
                  <div>
                    ...
                    <div className="font-primary text-xl font-bold">{listingPrice} ETH</div>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <div className="font-medium text-success">{netAmount} ETH</div>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <ul className="text-sm text-blue-800 space-y-1">
                      <li>• Your ticket will be transferred to escrow until sold</li>
                      <li>• You can cancel the listing anytime before it sells</li>
                      <li>• Platform fees are deducted from the sale price</li>
                      <li>• Payment is processed securely through smart contracts</li>
                    </ul>
                  </div>
                </div>
              </CardContent>
