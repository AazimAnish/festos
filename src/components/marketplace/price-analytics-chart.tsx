...
export function PriceAnalyticsChart({ 
  ...
  const data = useMemo(() => {
    const numberOfDays = selectedTimeFrame === '7d' ? 7 : 
                       selectedTimeFrame === '30d' ? 30 : 
                       selectedTimeFrame === '90d' ? 90 : 180;
  ...
  const chartData = {
    labels: data.map(point => {
      ...
      if (selectedTimeFrame === '30d') {
        ...
        return date.getDate() % 5 === 0 ? new Date(point.date).toLocaleDateString(undefined, { month: 'short', day: 'numeric' }) : '';
      }
      
      // For 90d or all, show monthly labels
      const date = new Date(point.date);
      return date.getDate() === 1 ? new Date(point.date).toLocaleDateString(undefined, { month: 'short' }) : '';
    }),
    datasets: [
      {
  ...
  const stats = useMemo(() => {
    if (!data.length) return { avgPrice: 0, totalVolume: 0, priceChange: 0, priceChangePercent: 0 };
    
    const avgPrice = data.reduce((sum, item) => sum + item.avgPrice, 0) / data.length;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
        <div className="space-y-1">
          ...
          <div className="font-primary text-lg font-bold">{stats.avgPrice} ETH</div>
        </div>
        <div className="space-y-1">
          <span className="text-sm text-muted-foreground">Total Volume</span>
          <div className="font-primary text-lg font-bold">{stats.totalVolume} tickets</div>
        </div>
        <div className="space-y-1">
          <span className="text-sm text-muted-foreground">Price Change</span>
          <div className={`font-primary text-lg font-bold ${stats.priceChange >= 0 ? 'text-success' : 'text-destructive'}`}>
            {stats.priceChange >= 0 ? '+' : ''}{stats.priceChange} ETH
          </div>
        </div>
        <div className="space-y-1">
          <span className="text-sm text-muted-foreground">Change %</span>
          <div className={`font-primary text-lg font-bold ${stats.priceChangePercent >= 0 ? 'text-success' : 'text-destructive'}`}>
