...
import { LocationDialog } from '@/shared/components/ui/location-dialog';
...
export function MapView({
  ...
  useEffect(() => {
    ...
    try {
      ...
      map.current = new maplibregl.Map({
        ...
        center: userLocation
          ...
          : [DEFAULT_LOCATION.lng, DEFAULT_LOCATION.lat], // Use user location or default
        zoom: 8,
    ...
    map.current.on('load', () => {
      ...
      events.forEach(event => {
        ...
        const eventIdNum = typeof event.id === 'string' ? parseInt(event.id) || 0 : event.id;
        const coordinates: [number, number] = [
          DEFAULT_LOCATION.lng + eventIdNum * 0.1,
          DEFAULT_LOCATION.lat + eventIdNum * 0.1,
        ];
                ...
                <div class="p-4 max-w-sm space-y-4">
                  <div class="space-y-3">
                    ...
                    <div class="flex items-center gap-2 text-sm text-muted-foreground">
                      ...
                      <span class="font-secondary">${new Date(
                        event.date
  ...
  const handleLocationGranted = (coords: { lat: number; lng: number }) => {
    ...
    toast.success('📍 Location access granted! Showing events near you.', {
      description:
        "We'll use your location to show personalized event recommendations.",
      duration: 4000,
    });
  };
  ...
  return (
    <>
      ...
      {/* Mobile Overlay */}
      <div
        className='fixed inset-0 bg-foreground/50 z-40 sm:hidden'
        onClick={onClose}
      />
        ...
        <div className='absolute top-0 left-0 right-0 z-10 bg-background/95 backdrop-blur-md border-b border-border p-3 sm:p-4'>
          <div className='flex items-center justify-between'>
            <div className='flex items-center gap-3'>
              ...
              <div>
                <h2 className='font-primary text-lg font-bold text-foreground tracking-tight'>
                  Map View
                </h2>
        ...
        {mapError ? (
          ...
          >
            <div className='text-center space-y-4'>
              <h2 className='font-primary text-xl font-bold text-foreground tracking-tight'>
                Map Error
              </h2>
              <p className='font-secondary text-sm text-muted-foreground tracking-tight'>
                {mapError}
