...
import { formatPrice } from '@/shared/utils/event-helpers';
...
export const EventCard = memo(function EventCard({
  ...
  const formatDate = useCallback((dateString: string) => {
    ...
    return {
      ...
      relative:
        diffDays > 0 ? `${diffDays}d` : diffDays === 0 ? 'Today' : 'Past',
        ...
        <div className={imageClasses}>
          ...
          {status !== 'confirmed' && (
            <div className='absolute top-3 right-3'>
              <Badge
                className={cn(
                  'backdrop-blur-sm shadow-apple-sm',
          ...
          >
            {saved ? (
              <BookmarkCheck
                className={cn(isHero ? 'w-5 h-5' : 'w-4 h-4', 'text-primary')}
              />
            ) : (
              <Bookmark
                className={cn(
          ...
          >
            ...
            <Button
              ...
              className={cn(
                ...
                isHero &&
                  'h-9 px-4 text-sm bg-primary text-primary-foreground hover:bg-primary/90',
                isList &&
                  'h-7 px-3 text-xs bg-accent/40 text-foreground hover:bg-primary hover:text-primary-foreground'
              )}
            >
