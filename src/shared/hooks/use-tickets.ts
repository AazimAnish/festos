...
export function useTickets() {
    ...
    async (
      ...
      try {
        ...
        const newListing: TicketListing = {
          ...
          price,
          originalPrice:
            mockOwnedTickets.find(t => t.id === ticketId)?.originalPrice ||
            '0 ETH',
          ...
          eventName:
            mockOwnedTickets.find(t => t.id === ticketId)?.eventName || '',
          eventDate:
            mockOwnedTickets.find(t => t.id === ticketId)?.eventDate || '',
          eventLocation:
            mockOwnedTickets.find(t => t.id === ticketId)?.eventLocation || '',
          eventImage:
            mockOwnedTickets.find(t => t.id === ticketId)?.eventImage || '',
          category: 'Tech', // Default category
          ticketType:
            mockOwnedTickets.find(t => t.id === ticketId)?.ticketType ||
            'General',
  ...
  const purchaseTicket = useCallback(
    async (listingId: string, price: string): Promise<TicketPurchase> => {
      if (!isConnected || !address) {
        throw new Error('Wallet not connected');
      }
      ...
      try {
        ...
        const totalAmount = (
          parseFloat(price) + parseFloat(serviceFee)
        ).toFixed(3);
  ...
  const getTransferableTickets = useCallback(() => {
    return ownedTickets.filter(
      ticket => ticket.transferable && ticket.status === 'valid'
    );
  }, [ownedTickets]);
