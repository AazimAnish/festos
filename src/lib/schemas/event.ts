...
const baseEventSchema = z.object({
  title: z
    ...
    .max(EVENT_CONFIG.MAX_TITLE_LENGTH, VALIDATION_MESSAGES.TOO_LONG('Title', EVENT_CONFIG.MAX_TITLE_LENGTH))
    .trim(),
  
  description: z
    .string()
    .min(1, VALIDATION_MESSAGES.REQUIRED)
    .max(EVENT_CONFIG.MAX_DESCRIPTION_LENGTH, VALIDATION_MESSAGES.TOO_LONG('Description', EVENT_CONFIG.MAX_DESCRIPTION_LENGTH))
    .trim(),
  
  location: z
    .string()
    .min(1, VALIDATION_MESSAGES.REQUIRED)
    .max(EVENT_CONFIG.MAX_LOCATION_LENGTH, VALIDATION_MESSAGES.TOO_LONG('Location', EVENT_CONFIG.MAX_LOCATION_LENGTH))
    .trim(),
  
  startDate: z
    .string()
    .datetime(VALIDATION_MESSAGES.INVALID_DATE),
  ...
  maxCapacity: z
    ...
    .max(EVENT_CONFIG.MAX_CAPACITY, `Capacity cannot exceed ${EVENT_CONFIG.MAX_CAPACITY}`),
  ...
  timezone: z
    .string()
    .default(EVENT_CONFIG.DEFAULT_TIMEZONE),
...
export const createEventSchema = baseEventSchema.extend({
  ...
  privateKey: z
    .string()
    .optional(), // For server-side contract interaction
  useTestnet: z
    .boolean()
    .default(true), // Default to testnet for safety
});
