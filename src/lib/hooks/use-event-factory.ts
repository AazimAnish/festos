...
import { getContract, parseEther, formatEther, type Abi, type Client } from 'viem';
import type {
	Event,
	Ticket,
	EventStats,
...
interface UseEventFactoryReturn {
	...
	getUserTicketId: (eventId: bigint, userAddress: `0x${string}`) => Promise<bigint>;
	...
	purchaseTicket: (eventId: bigint, ticketPrice: string) => Promise<`0x${string}` | null>;
	...
	canPurchaseTicket: (event: Event, userAddress: `0x${string}`) => Promise<boolean>;
}

export function useEventFactory(contractAddress?: `0x${string}`): UseEventFactoryReturn {
	...
	// Read functions
	const getEvent = useCallback(async (eventId: bigint): Promise<Event | null> => {
	...
	const getTicket = useCallback(async (ticketId: bigint): Promise<Ticket | null> => {
		if (!contract) return null;
		try {
			setIsLoading(true);
			setError(null);
	...
	const getEventStats = useCallback(async (eventId: bigint): Promise<EventStats | null> => {
		if (!contract) return null;
		try {
			setIsLoading(true);
			setError(null);
	...
	const getUserEvents = useCallback(async (addr: `0x${string}`): Promise<bigint[]> => {
		if (!contract) return [];
		try {
			setIsLoading(true);
			setError(null);
			...
			const errorMessage = err instanceof Error ? err.message : 'Failed to get user events';
			setError(errorMessage);
	...
	const getUserTickets = useCallback(async (addr: `0x${string}`): Promise<bigint[]> => {
		if (!contract) return [];
		try {
			setIsLoading(true);
			setError(null);
			...
			const errorMessage = err instanceof Error ? err.message : 'Failed to get user tickets';
			setError(errorMessage);
	...
	const hasTicket = useCallback(async (eventId: bigint, addr: `0x${string}`): Promise<boolean> => {
		...
		try {
			...
			const errorMessage = err instanceof Error ? err.message : 'Failed to check ticket status';
			setError(errorMessage);
	...
	const getUserTicketId = useCallback(async (eventId: bigint, addr: `0x${string}`): Promise<bigint> => {
		...
		try {
			...
			const errorMessage = err instanceof Error ? err.message : 'Failed to get user ticket ID';
			setError(errorMessage);
	...
	const getTotalEvents = useCallback(async (): Promise<bigint> => {
		...
		try {
			...
			const errorMessage = err instanceof Error ? err.message : 'Failed to get total events';
			setError(errorMessage);
	...
	const getTotalTickets = useCallback(async (): Promise<bigint> => {
		...
		try {
			...
			const errorMessage = err instanceof Error ? err.message : 'Failed to get total tickets';
			setError(errorMessage);
	...
	const getPlatformFee = useCallback(async (): Promise<bigint> => {
		...
		try {
			...
			const errorMessage = err instanceof Error ? err.message : 'Failed to get platform fee';
			setError(errorMessage);
	...
	// Write functions
	const createEvent = useCallback(async (params: CreateEventParams): Promise<`0x${string}` | null> => {
		if (!contract) return null;
		try {
			setIsLoading(true);
			setError(null);
	...
	const purchaseTicket = useCallback(async (eventId: bigint, ticketPrice: string): Promise<`0x${string}` | null> => {
		if (!contract) return null;
		try {
			setIsLoading(true);
			setError(null);
	...
	const approveTicket = useCallback(async (ticketId: bigint): Promise<`0x${string}` | null> => {
		if (!contract) return null;
		try {
			setIsLoading(true);
			setError(null);
	...
	const useTicket = useCallback(async (ticketId: bigint): Promise<`0x${string}` | null> => {
		if (!contract) return null;
		try {
			setIsLoading(true);
			setError(null);
	...
	const updateEvent = useCallback(async (params: UpdateEventParams): Promise<`0x${string}` | null> => {
		if (!contract) return null;
		try {
			setIsLoading(true);
			setError(null);
	...
	const cancelEvent = useCallback(async (eventId: bigint): Promise<`0x${string}` | null> => {
		if (!contract) return null;
		try {
			setIsLoading(true);
			setError(null);
	...
	const isEventFull = useCallback((event: Event): boolean => {
		return event.maxCapacity > BigInt(0) && event.currentAttendees >= event.maxCapacity;
	}, []);
