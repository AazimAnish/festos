  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-10">
      <div className="text-center space-y-4">
        <h2 className="font-primary text-xl font-bold text-foreground">Failed to load events</h2>
        <p className="font-secondary text-sm text-gray">Please try refreshing the page or check your connection.</p>
      </div>
    </div>
  )
}
  ...
  const filteredEvents = useMemo(() => {
    ...
    return SAMPLE_EVENTS.filter((event) => {
      ...
      if (filters.priceRange.min > 0 || filters.priceRange.max < 1) {
        const eventPrice = event.price === "Free" ? 0 : Number.parseFloat(event.price.split(" ")[0])
